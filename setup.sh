...
apt-get install -y python3 python-pip build-essential libffi-dev python-dev ||
	dnf install -y python3 python-pip build-essential libffi-dev python-dev ||
	yum install -y python3 python-pip build-essential libffi-dev python-dev ||
	pacman -Syu --noconfirm python3 python-pip build-essential libffi-dev python-dev ||
...
mkdir -p /home/jail/log && mkdir /home/jail/cache && mv *.py /home/jail;
...
mv wordlist.txt /home/jail && chmod 644 /home/jail/wordlist.txt;
echo -e "#!/bin/bash\n\nbash;" > /usr/local/bin/test && chmod +x /usr/local/bin/test;
echo "\nDone.\n";
